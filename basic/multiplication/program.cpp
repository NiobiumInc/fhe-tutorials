 ...
 *
 *   $ niobium run basic/multiplication 5.981 -7.033997655
 *   <snip>
...
void multiply(CryptoContext<DCRTPoly> cc, Ciphertext<DCRTPoly> ct1, Ciphertext<DCRTPoly> ct2, Ciphertext<DCRTPoly>& multiplication) {
  multiplication = cc->EvalMult(ct1, ct2);
}
...
int main(int argc, char** argv) {
  ...
  Plaintext multPt;
  cc->Decrypt(keys.secretKey, multiplication, &multPt);
  multPt->SetLength(1);

  auto d = multPt->GetCKKSPackedValue()[0].real();
